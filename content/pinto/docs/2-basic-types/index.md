...
The boolean type can have exactly two distinct values, and has the following literals:

In pint°, those values are represented by the literals `true` and `false`.

### Numbers

pint° follows Dart's approach of having `int` and `double` as the numerical
types[^2]. Their literals are also similar to Dart's.

For integers, we use number characters directly. pint° has support for
...
For doubles, pint° uses the commonly used dot notation: `0.01`. Unlike
...
readable to omit the leading zero. As a plus, we have a literal syntax
...
are nameless data structures whose identity is defined by their shape and
values, unlike a standard class in an object-oriented language.
...
is similar to Dart, but they use the symbol literals to describe the name of
the field:
...
Struct members **can** have the field name omitted. This doesn't mean that the
field is nameless, as happens with Dart records. Instead, a synthetic name is
generated in the format `$<index>`. This is mostly so we can interoperate
...
Not only may the name of a field be omitted, but its value can also be omitted.
...
information it carries is its own existence.

If you have never used a language that has a unit type (there are many of them),
it's used when you don't want to convey any information. For instance, in Dart,
functions that return "nothing"[^1] have a return type of `void`. In pint°,
